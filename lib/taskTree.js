...
const {
	forOwn, getGulp, getModule, isFunction, isObject, isString,
	makeArray, parseParameters, pick
} = require('./util');
...
function _getTask(taskPath, taskId, cwd, gulp) {
	...
	if (!task.watch && (task.fn.length === 1) && (isFunction(task.fn[0]))) {
		...
		task.fn = (done)=>fn(...getInjection(fn, cwd, {gulp:_gulp, done}));
...
async function taskTree(roots=[], {filter=xJsFilter, gulp=getGulp(4)}) {
	const tree = directoryTree(roots, filter);
